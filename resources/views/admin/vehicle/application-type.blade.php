...
@section('app_type','active')
@section('content')

@include('vrms2.mod3-submenu')
<h3>
      {{trans('title.applicationtype')}}
...
<div class="card-body">
   <table id="myTable" class="table table-striped" style="width:100%">
      ...
      <tbody>
         @foreach($applicationtype as $data) 
         <tr>
            <td>{{$data->name}}</td>
            <td>{{$data->name_en}}</td>
            <td>@if($data->status ==1){{trans('table.active')}} @else {{trans('table.deactive')}} @endif</td>
            <td class="sorting"> 
               <button type="button" class="btn btn-info edit_btn" data-toggle="modal" data-backdrop="static" data-keyboard="false" data-target="#editModel" data-act="Edit" data-name="{{$data->name}}" data-name_en="{{$data->name_en}}" data-status="{{$data->status}}" data-id="{{$data->id}}">{{trans('button.edit')}}
               </button>
...
@component('component.admin.applicationtype') @endcomponent 
@include('delete') 
@endsection 
@push('page_scripts') 
<script type="text/javascript">
 
   var base_url = "{{url('admin/application-type')}}";
