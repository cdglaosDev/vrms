...
$user =\App\User::get();
@endphp
    <h1 class="page-header">{{trans('transfer_vehicle.update_vehicle_inspection')}}</h1>
    <div class="card">
        <div class="card-body">
        @include('flash') 
       
             <form action="{{route('vehicle-inspection.update',['id'=>$vehicle_inspection->id])}}" method="POST" enctype="multipart/form-data">
                               @method('patch')
                  ...
                  <select class="form-control js-example-basic-single" style="width: 100%;"   name="vehicle_id" required>
                     ...
                     @endforeach
                      ...
                      <div class="modal-footer">
                          <a href="/vehicle-inspection" class="btn btn-secondary btn-sm">{{trans('finance_button.cancel')}}</a>
