...
<div class="table-responsive">
    <table class="table vehicles_list fundClassesTable" id="vTable">
        ...
        <!-- When we use with <thead> for header row, we can adjust to equal column width header and detail columns. So, create header row in <tbody>. -->
            ...
            <tr>
                <td class="v_lic_no">
                    <a href="#" data-url="{{ route('editVehicle',['id'=>$vehicle->id])}}" class="link license_no" data-id="{{$vehicle->id}}" purpose_no="{{$vehicle->vehicle_kind_code}}" onclick="vehicleModal(this, 'Load')">
                ...
                <td class="v_t_name">
                    ...
                    <div style="color:red;font-weight: bold;font-size:11px !important;">{{ $vehicle->traffic_note ?? '' }}</div>
                </td>
                <td class="v-brand">
