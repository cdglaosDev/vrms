<style>
   ...
   .doc-certificate .l_height{
      height: 32px;
   }
</style>
<div class="card" style="width: 100%; border: none !important">
   <div class="card-body doc-certificate fs-1">
      ...
      <div style="line-height: 200%;">
         ...
         <div class="row fs-2 ml-2 l_height">
            <div style="width:20%;"><label for="">ຍີ່ຫໍ້ &nbsp; @if($vehicle->brand_id)<b>{{ $vehicle->vbrand->name ?? ''}} </b>@endif</label></div>
            <div style="width:20%;"><label for="">ສີ &nbsp; @if($vehicle->color_id)<b>{{ $vehicle->color->name ?? '' }} </b>@endif</label></div>
            <div style="width:20%;"><label for="">ພວງມະໄລ: &nbsp;@if($vehicle->steering_id)<b>{{ $vehicle->steering->name ??'' }} </b>@endif </label></div>
         </div>
         <div class="row fs-2 ml-2 l_height">
            <div style="width:40%;"><label for="">ເລກຈັກ: &nbsp; <span style="font-weight: bold;font-family:Saysettha OT !important;">{{ $vehicle->engine_no ?? '' }}</span></label></div>
            <div style="width:40%;"><label for="">ເລກຖັງ: &nbsp; <span style="font-weight: bold;font-family:Saysettha OT !important;">{{ $vehicle->chassis_no ?? '' }}</span></label></div>
         </div>
         <div class="row fs-2 ml-2 l_height">
            <div style="width:40%;"><label for="">ຊື່ເຈົ້າຂອງທະບຽນ &nbsp; <b>{{ $vehicle->owner_name ?? '' }} </b></label></div>
            <div style="width:25%;"><label for="">ບ້ານ: &nbsp; @if($vehicle->village_name) <b>{{ $vehicle->village_name ?? '' }} </b> @endif</label></div>
            <div style="width:25%;"><label for="">ເມືອງ: &nbsp; @if($vehicle->district_code)<b>{{ $vehicle->district->name }} </b> @endif</label></div>
         </div>
