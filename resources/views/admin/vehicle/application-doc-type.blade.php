...
@section('app_doc','active')
@section('content')
@include('vrms2.mod3-submenu')
<h3>
   {{trans('title.applicationdoctype')}}
...
<div class="card-body">
   <table id="myTable" class="table table-striped" style="width:100%">
      ...
      <tbody>
         @foreach($applicationdoctype as $data)
         <tr>
            <td>{{$data->name}}</td>
            <td>{{$data->name_en}}</td>
            ...
            <td class="sorting">
               ...
               data-target="#deleteModel" data-act="Delete" data-id="{{$data->id}}">
...
@component('component.admin.applicationdoctype') @endcomponent
@include('delete')
@endsection
@push('page_scripts')
<script type="text/javascript">
   var base_url = "{{url('admin/application-doc-type')}}";
   ...
   function insertEngineBrand(name, name_en, form, oldId = null) {
      var url = "/get-appdoc?name=" + name.val() + "&name_en=" + name_en.val() + "&id=" + oldId;
