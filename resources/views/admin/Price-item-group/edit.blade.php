...
@endphp
    <h1 class="page-header">{{trans('book.price_item_gruop_update')}}</h1>
    <div class="panel panel-inverse">
    @include('flash') 
        <div class="body">
            <form action="{{route('Price-item-group.update',['id'=>$datas->id])}}" method="POST" enctype="multipart/form-data">
                    @method('PATCH')
                      @csrf
                    ...
                    <div class="row pb-3">
                        ...
                        <div class="col-sm-4">
                            <div class="from-group">
                                ...
                                <input type="text" name="group_code" id="validationCustom01" class="form-control" value="{{$datas->group_code}}" placeholder="{{trans('book.enter_group_code')}}" required="">
                            </div>
                        </div>
                        ...
                        <div class="col-sm-4">
                            <div class="from-group">
                                ...
                                <input type="text" name="group_name" id="validationCustom01" class="form-control" value="{{$datas->group_name}}" placeholder="{{trans('book.enter_group_code_name')}}" required="">
                            </div>
                        </div>
                            <div class="col-sm-4">
                                <div class="from-group">
                                    <label for="name">{{trans('book.group_name_en')}}:</label>
                                    <input type="text" name="group_name_en" id="validationCustom01" class="form-control" value="{{$datas->group_name_en}}" placeholder="{{trans('book.enter_group_code_name_en')}}" required="">
                                </div>
                            </div>
                        ...
                        <div class="row pb-3">
                            ...
                            <div class="col-sm-4">
                                <div class="from-group">
                                    ...
                                    <input type="text" name="group_note" id="validationCustom01" class="form-control" value="{{$datas->group_note}}" placeholder="{{trans('book.enter_group_note')}}" required="">
                                </div>
                            </div>
        ...
        <div class="row pList table-responsive">
                <table class="table table-striped table-bordered table-hover">
                    ...
                    <div class="col-md-12"> 
                     ...
                     <span class="input-group-addon" id="sizing-addon1">{{trans('finance_title.add_price_item')}}</span>
                     
                             
                           <select name="price_item_id[]" id="sizing-addon1" class="form-control" required="" >
                                        	 
                                            @foreach ($item as $imp_country)
                                                <option value="{{$imp_country-> id}}" {{ $datas->price_item_id == $imp_country->id ? 'selected' : '' }} >{{$imp_country->name}}<span>({{$imp_country->name_en}})</span></option>
                                            @endforeach
                                        </select>
                   ...
                   <a href="{{route('Price-item-group.index')}}" class="btn btn-secondary">{{trans('finance_button.cancel')}}</a>
