...
@section('working_status','active')
@section('content')

@include('vrms2.mod3-submenu')
<h3>
   {{trans('title.workingstatus')}}
   ...
   <table id="myTable" class="table table-striped" style="width:100%">
      <thead>
         <tr>
            ...
            <th>{{trans('table.workingstatusgroup')}}</th>
            <th>{{trans('table.status')}}</th>
            <th>{{trans('table.action')}}</th>
         </tr>
      </thead>
      <tbody>
         @foreach($workingstatus as $data) 
         <tr>
            <td>{{$data->name}}</td>
            <td>{{$data->name_en}}</td>
            <td>{{$data->description}}</td>
            <td>@if(isset($data->group->name)) <span>{{$data->group['name']}}({{$data->group['name_en']}})</span>@else{{"_"}}@endif </td>
            <td>@if($data->status ==1){{trans('table.active')}} @else {{trans('table.deactive')}} @endif</td>
            <td class="sorting"> 
               <button type="button" class="btn btn-info edit_btn" data-toggle="modal" data-backdrop="static" data-keyboard="false" data-target="#editModel" data-act="Edit" data-name="{{$data->name}}" data-name_en="{{$data->name_en}}" data-description="{{$data->description}}" data-working_status_group_id="{{$data->working_status_group_id}}" data-status="{{$data->status}}" data-id="{{$data->id}}">{{trans('button.edit')}}
               </button> 
...
@component('component.admin.workingstatus',['working'=>$working]) @endcomponent
@include('delete')
@endsection 
@push('page_scripts') 
<script type="text/javascript">
  
   var base_url = "{{url('admin/working-status')}}";
   ...
   $(document).on("click", '.edit_btn', function(e) {
     ...
     $('[name="working_status_group_id"]').val($(this).data('working_status_group_id'));
