...
@section('content')
<h3>
   Attache Application Document File
</h3>
@if(!empty(session('duplicate_data')))
   <p class="alert alert-danger duplicate">{{ trans('importer.import_status') }}</p>
@else
   <p class="alert alert-info duplicate" >Import Data Success.</p>
@endif
@include('flash');
<div class="card-body">
         ...
         @if(!empty(session('vehicle_id')))
            @foreach(session('vehicle_id') as $vehicle)
               ...
               <tr>
                  ...
                  <td><a data-url="{{ url('/attach-document-import',['id'=>$detail->id])}}" data-id="{{ $detail->id }}" data-license_no_need="{{ $detail->licence_no_need}}" class="btn btn-info show_btn attach" data-backdrop="static" data-keyboard="false"  data-toggle="modal"  data-target="#showModal" >Attach Document</a></td>
               </tr>
               @endforeach
            @endforeach
         @endif
      ...
      @if(!empty(session('duplicate_data')))
         <h5 style="color:red">Duplicate Data following:</h5>
            @foreach(session('duplicate_data') as $index=>$data)
              <div class="col-md-12">
              {{ $index+1 }}. Pre License = {{ $data['pre_license'] }}, Engine no = {{ $data['engine_no'] }}, Chassis no = {{ $data['chassis_no'] }}
              </div>
            @endforeach
      @endif
