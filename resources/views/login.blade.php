    ...
    <div class="row">
        <div style="width: 23%">
            ...
            <form action="" id="loginForm" method="POST">
				@csrf
                <div class="box1 w150" style="border:none;text-align:left;overflow-x:hidden;margin-top:27px;margin-left:27px">
				<div id="login-box0" class="tl login-tool form-table login_tool_set" success="home" style="padding:6px">
                <b>LoginId:</b><input type="text" class="login-username" id="login-username" name="login_id"><br>
                <b>Password:</b><input type="password" id="login-password" name="password"><br>
                <b></b><a  class="login-submit button" >Sign In</a><br>
                
                </div>
            </form>
        </div>
		<a class="btn btn-primary" style="width: 149px;margin: 10px 0px 5px 26px;">ລົງທະບຽນສໍາລັບລູກຄ້າສູນລົດ</a><br>
			<a href="{{ url('customer/register') }}" class="btn btn-primary" style="margin-left: 60px;">ລົງທະບຽນ</a>
    </div>
...
<div id="loginModal" class="modal custom-modal fade"  role="dialog">
  <div class="modal-dialog">
    <div class="modal-content">
	  ...
	  <div class="modal-body">
		<form action="{{ url('/protected-login') }}" id="login-form" method="POST">
		@csrf
		
	  <h5>This account already login in.</h5><br/>
		
		<input type="hidden" name="login_id" value="" class="login_id">
...
$(function(){
	...
	$(".login-submit").click(function(){
		var login_id = $('.login-username').val();
		var password = $('#login-password').val();
		if(login_id && password){
				$.ajax({
				url:'/user-login',
				method:"POST",
				data:{login_id:login_id, password:password, _token:document.querySelector('meta[name="csrf-token"]').content},
				success:function(data){
					switch (data.status) {
					case "pending":
						alert('Your account is not active.Please contact Officer.');
						break;
					case "logined":
					...
					case "invalid":
						...
						break
					default:
					window.location.href = '/announcement-page-list';
					}
					
				}
				});
		} else {
			alert('You need to fill loginId and password');
		}
